...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/11/15 18:28:08 by stales            #+#    #+#             */
/*   Updated: 2024/11/19 14:38:15 by stales              1993-2024            */
/*                                                                            */
/* ************************************************************************** */
...
aes_status_t	aes_256_cbc_enc(byte_t *out, size_t o_sz, iv_t iv, const byte_t *restrict in, size_t i_sz, const aes_ctx_t *ctx)
{
	...
	for (i = 0; i < blocks; i++) {
		...
		feedback = AddRoundKey(feedback, ctx->key.sched_256[0]);
		
		for (j = 1; j < AES_256_NR; j++) {
			...
			feedback = _mm_aesenc_si128(feedback, ctx->key.sched_256[j]);
		}
		...
		feedback = _mm_aesenclast_si128(feedback, ctx->key.sched_256[j]);
...
aes_status_t	aes_256_cbc_dec(byte_t *out, size_t o_sz, iv_t iv, const byte_t *restrict in, size_t i_sz, const aes_ctx_t *ctx)
{
	...
	for (i = 0; i < blocks; i++) {
        ...
        state = AddRoundKey(state, ctx->key.sched_256[AES_256_NR]);

        for (j = ~-AES_256_NR; j > 0; j--) {
            ...
            state = _mm_aesdec_si128(state, _mm_aesimc_si128(ctx->key.sched_256[j]));
		}
        
        state = _mm_aesdeclast_si128(state, ctx->key.sched_256[0]);
