...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/11/15 18:27:49 by stales            #+#    #+#             */
/*   Updated: 2024/11/19 14:55:49 by stales              1993-2024            */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (!ctx || !out || !in || (o_sz < i_sz) || ctx->key_size != AES_KEY_128)
		return (AES_ERR);
	...
	for (i = 0; i < blocks; i++) {
		...
		feedback = AddRoundKey(feedback, ctx->key.sched_128[0]);
		...
		for (j = 1; j < AES_128_NR; j++) {
			...
			feedback = _mm_aesenc_si128(feedback, ctx->key.sched_128[j]);
		}
		...
		feedback = _mm_aesenclast_si128(feedback, ctx->key.sched_128[j]);
...
aes_status_t	aes_128_cbc_dec(byte_t *out, size_t o_sz, iv_t iv, const byte_t *restrict in, size_t i_sz, const aes_ctx_t *ctx)
{
	...
	for (i = 0; i < blocks; i++) {
        ...
        state = AddRoundKey(last_in, ctx->key.sched_128[AES_128_NR]);

        for (j = ~-AES_128_NR; j > 0; j--) {
