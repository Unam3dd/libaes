...
/*   Updated: 2024/10/25 19:39:33 by stales           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static inline aes_round_key_t AES_128_ASSIST(aes_round_key_t tmp1, aes_round_key_t tmp2)
{
	aes_round_key_t tmp3 = _mm_setzero_si128();
...
{
	aes_round_key_t tmp1 = _mm_setzero_si128();
	...
	tmp2 = _mm_aeskeygenassist_si128((__m128i)tmp1, 0x1);
	tmp1 = AES_128_ASSIST(tmp1, tmp2);
	sched[1] = tmp1;

	// Round 2 - RC[2] = 0x2
	tmp2 = _mm_aeskeygenassist_si128((__m128i)tmp1, 0x2);
	tmp1 = AES_128_ASSIST(tmp1, tmp2);
	sched[2] = tmp1;

	// Round 3 - RC[3] = 0x4
	tmp2 = _mm_aeskeygenassist_si128((__m128i)tmp1, 0x4);
	tmp1 = AES_128_ASSIST(tmp1, tmp2);
	sched[3] = tmp1;


	// Round 4 - RC[4] = 0x8
	tmp2 = _mm_aeskeygenassist_si128((__m128i)tmp1, 0x8);
	tmp1 = AES_128_ASSIST(tmp1, tmp2);
	sched[4] = tmp1;


	// Round 5 - RC[5] = 0x10
	tmp2 = _mm_aeskeygenassist_si128((__m128i)tmp1, 0x10);
	tmp1 = AES_128_ASSIST(tmp1, tmp2);
	sched[5] = tmp1;


	// Round 6 - RC[6] = 0x20
	tmp2 = _mm_aeskeygenassist_si128((__m128i)tmp1, 0x20);
	tmp1 = AES_128_ASSIST(tmp1, tmp2);
	sched[6] = tmp1;


	// Round 7 - RC[7] = 0x40
	tmp2 = _mm_aeskeygenassist_si128((__m128i)tmp1, 0x40);
	tmp1 = AES_128_ASSIST(tmp1, tmp2);
	sched[7] = tmp1;


	// Round 8 - RC[8] = 0x80
	tmp2 = _mm_aeskeygenassist_si128((__m128i)tmp1, 0x80);
	tmp1 = AES_128_ASSIST(tmp1, tmp2);
	sched[8] = tmp1;


	// Round 9 - RC[9] = 0x1b
	tmp2 = _mm_aeskeygenassist_si128((__m128i)tmp1, 0x1b);
	tmp1 = AES_128_ASSIST(tmp1, tmp2);
	sched[9] = tmp1;

	// Round 10 - RC[10] = 0x36
	tmp2 = _mm_aeskeygenassist_si128((__m128i)tmp1, 0x36);
	tmp1 = AES_128_ASSIST(tmp1, tmp2);
	sched[10] = tmp1;
}
