...
/*   Updated: 2024/11/15 13:48:21 by stales           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
aes_status_t	aes_256_ecb_enc(byte_t *out, size_t o_sz, const byte_t *restrict in, size_t i_sz, const aes_ctx_t *ctx)
{
	...
	for (i = 0; i < blocks; i++) {
		...
		state = AddRoundKey(state, ctx->key.sched_256[0]);
		...
		for (j = 1; j < AES_256_NR; j++) {
			...
			state = _mm_aesenc_si128(state, ctx->key.sched_256[j]);
		}

		//      a[127:0] := ShiftRows(a[127:0])
		//      a[127:0] := SubBytes(a[127:0])
		//      dst[127:0] := a[127:0] (AddRoundKey) XOR RoundKey[127:0]
		state = _mm_aesenclast_si128(state, ctx->key.sched_256[j]);
