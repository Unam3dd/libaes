...
int main(void)
{
	...
	for (size_t i = 0; i < 0x44; i++)
		in[i] = (uint8_t)((i * 11 + 31) & 0xFF);

	strncpy((char *)cpy, (char *)in, sizeof(cpy));
	
	pkcs_status_t status = PKCS_OK;

	if ((status = pkcs_pad(in, sizeof(in), 0x44)) != PKCS_OK) {
		fprintf(stderr, "Error: pkcs_pad() = %d\n", status);
		return 1;
	}

	printf("=== AES-256-CFB Test 2 ===\n");

	// 256-bit key
	for (size_t i = 0; i < 32; i++)
		ctx.key.key_256[i] = (uint8_t)((i * 13 + 5) & 0xFF);

	// IV
	memset(iv, 0, sizeof(iv_t));
	memcpy(iv, "CFB-MODE-256BIT", 15);
	memcpy(iv_copy, iv, sizeof(iv_t));

	printf("AES-256-CFB Key (first 16 bytes): ");
	...
	aes_cfb_enc(out, sizeof(out), iv, in, sizeof(in), &ctx);
	...
	aes_cfb_dec(out, sizeof(out), iv, in, sizeof(in), &ctx);
	...
	printf("✓ AES-256-CFB Test 2 PASSED!\n");
	return 0;
}
