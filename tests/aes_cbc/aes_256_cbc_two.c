...
int main(void)
{
	...
	for (size_t i = 0; i < 0x50; i++)
		in[i] = (uint8_t)((i ^ 0xAA) & 0xFF);

	strncpy((char *)cpy, (char *)in, sizeof(cpy));
	
	pkcs_status_t status = PKCS_OK;

	if ((status = pkcs_pad(in, sizeof(in), 0x50)) != PKCS_OK) {
		fprintf(stderr, "Error: pkcs_pad() = %d\n", status);
		return 1;
	}

	printf("=== AES-256-CBC Test 2 ===\n");
	printf("Input size: %zu bytes\n", sizeof(in));

	// 256-bit key
	for (size_t i = 0; i < 32; i++)
		ctx.key.key_256[i] = (uint8_t)((i * 5 + 17) & 0xFF);

	// IV
	for (size_t i = 0; i < 16; i++)
		iv[i] = (uint8_t)((i * 13 + 3) & 0xFF);

	memcpy(iv_copy, iv, sizeof(iv_t));

	printf("AES-256-CBC Key : ");
	...
	aes_cbc_enc(out, sizeof(out), iv, in, sizeof(in), &ctx);

	printf("Output after encryption = ");
	print_hex(out, 32); // Print first 32 bytes

	memcpy(in, out, sizeof(in));
	memset(out, 0, sizeof(out));
	memcpy(iv, iv_copy, sizeof(iv_t)); // Restore IV

	aes_cbc_dec(out, sizeof(out), iv, in, sizeof(in), &ctx);

	printf("Output after decryption (first 32 bytes) = ");
	print_hex(out, 32);
	...
	printf("✓ AES-256-CBC Test 2 PASSED!\n");
	return 0;
}
